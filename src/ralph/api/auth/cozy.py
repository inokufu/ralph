...
def get_cozy_user(
    ...
    if x_auth_token is None or "Bearer" not in x_auth_token:
        logger.debug(
            "Not using cozy auth which requires a Bearer token"
        )
        return None

    if cookie is None or "cozysessid" not in cookie:
        logger.debug(
            "Not using cozy auth which requires a cozysessid cookie"
        )
        return None
    ...
    # TODO: Do we need a specific agent for Cozy instead of BaseXapiAgentWithOpenId ?
